...
public class ChessApp extends Application {
    ...
    private boolean blackKingMoved = false;              // these booleans helps for castling move(rok atma)
    ...
    public void handleDragStarted(Piece piece) {
        if (selectedPiece != null && selectedPiece != piece) { // if you selected a piece already and you try to drag another piece,

            selectedPiece.setStyle("");                    // clear the selected piece's yellow border color
        }
        clearValidMoves(); // it clears the valid moves for the selected piece ( with green dots )
    ...
    public void handleDragDone(Piece piece, boolean dropCompleted) {
        clearValidMoves(); // clear the valid moves for the selected piece ( with green dots )

        if (this.selectedPiece == piece) { // it checks if the selected piece is the same as the dragged piece
            if (!dropCompleted) { // if the drag move is failed, clear the selected piece's yellow border color
                piece.setStyle("");
            }
            this.selectedPiece = null; // drag move is completed,clear the selectedPiece
        }
    }
        ...
        if (canMovePiece(selectedPiece, targetRow, targetCol)) { // we call a canMovePiece method and if it returns true, it means that the move is possible
            ...
            if (pieceThatMoved instanceof Pawn pawn) { // if the selected piece is a pawn
                ...
                if (pawn.getRow() == promotionRank) { // if the selected piece is a pawn and it's on the promotion row
                    ...
                    if (promotedPiece != null) { // if player chose a piece to promote
            ...
            selectedPiece.setStyle(""); // after the valid move clear yellow border color of the selected piece
        ...
        if (targetTileNode != null) { // if the target square is existing
            for (Node node : targetTileNode.getChildren()) { // look inside the target square
                if (node instanceof Piece && node != piece) { // if the target square is occupied by a piece and it's not the selected piece
                    Piece occupyingPiece = (Piece) node; // lets call that peice as occupyingPiece
    ...
    private void checkGameStateAfterMove() { // this method checks game situation after moves
        if (isKingInCheck(whiteTurn)) {
            String currentPlayerName = whiteTurn ? player1 : player2; // find the player who is in check
                ...
                tile.setOnDragDropped(event -> { // something dropped on this square
                    ...
                    if (db.hasString() && selectedPiece != null) {
                        ...
                        if (tryMoveSelectedPiece(targetR, targetC)) { // we need to ask that method can we take that selected piece to target square ?
                            ...
                            if(selectedPiece != null) selectedPiece.setStyle("");
                        }
                    }
